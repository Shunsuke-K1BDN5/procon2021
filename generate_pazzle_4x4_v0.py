...
class puzzle():
    ...
    def make_pic(self,target,type = "nomal"):
        ...
        Image.fromarray(deffrences.astype(np.uint8)).save(f"E:/procon/data/4x4_v0/{folder}_{f_num}/{label}.jpg")
        file_num[folder[0]][folder[1]] += 1
        # if  len(self.backwerd_rec.keys())  != 0:
        #      for folder in self.backwerd_rec.keys():
        #         label = file_num[folder[0]][folder[1]]
        #         f_num = folder[0]*4 + folder[1]
        #         deffrences = self.deffrence()
        #         deffrences = deffrences + self.place_dimension

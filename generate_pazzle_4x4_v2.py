...
class puzzle():
    ...
    def make_pic(self,target,type = "nomal"):
        ...
        f_num = folder[0]*4 + folder[1]
        """Image.fromarray(self.pazzle.astype(np.uint8)).save(f"E:/procon/data/4x4_v0/{folder}_{f_num}/{label}.jpg")"""
        torch.save(self.pazzle,f"E:/procon/data/4x4_v3/{folder}_{f_num}/{label}.pt")
        #torch.save(self.pazzle,f"E:/procon/data/4x4_v1/eve/1.pt")
...
file_num = [[0] * 4 for i in range(4)]

for i in range(150000):
    select = random.randint(2,8)
    pazzle = puzzle(4, 4, select, 10 ,"cartesian_coordinates")
    pazzle.create()
    # print(pazzle.fowerd_rec)
    # print(pazzle.backwerd_rec)
